...
def get_value_list(group):
    ...
    def _load():
        if group:
            ...
            return ()
    return const.LazyChoices(_load)
...
class DataImport(generic.BO):
    ...
    description = models.TextField(_('description'),blank=True,null=True)
    content_type = models.ForeignKey(ContentType,verbose_name=_("content type"), on_delete=models.CASCADE,limit_choices_to={"app_label__in":['basedata','organ','auth']})
