            ...
            with transaction.atomic():
                for code,name,description,unit_code,unit_name,warehouse_code,warehouse_name,price,cnt in reader:
                    ...
                    try:
                        ...
                        measure = Measure.objects.create(code=unit_code,name=force_str(unit_name.decode('gbk') if isinstance(unit_name, bytes) else unit_name))

                    try:
                        house = Warehouse.objects.get(code=warehouse_code)
                    except Exception as e:
                        house = Warehouse.objects.create(code=warehouse_code,name=force_str(warehouse_name.decode('gbk') if isinstance(warehouse_name, bytes) else warehouse_name))

                    try:
                        material = Material.objects.get(code=code)
                    except Exception as e:
                        material = Material(code=code,name=force_str(name.decode('gbk') if isinstance(name, bytes) else name),spec=force_str(description.decode('gbk') if isinstance(description, bytes) else description),warehouse=house)
...
class StockIn(generic.BO):
    ...
    status = models.CharField(_("status"),max_length=const.DB_CHAR_CODE_2,default='0',choices=STATUS)
    execute_time = models.DateTimeField(_("execute time"),blank=True,null=True)
    po = models.ForeignKey(PurchaseOrder,verbose_name=_("purchase order"), on_delete=models.CASCADE,null=True,limit_choices_to={"entry_status":"0"},blank=True)
...
class StockOut(generic.BO):
    ...
    org = models.ForeignKey(Organization,verbose_name=_("organization"), on_delete=models.CASCADE,blank=True,null=True)
    title = models.CharField(_("title"),max_length=const.DB_CHAR_NAME_40)
    project = models.ForeignKey(Project,verbose_name=_("project"), on_delete=models.CASCADE,blank=True,null=True)
    wo = models.ForeignKey(WorkOrder,verbose_name=_("work order"), on_delete=models.CASCADE,blank=True,null=True)
    description = models.TextField(_("description"),blank=True,null=True)
    amount = models.DecimalField(_("money of amount"),max_digits=14,decimal_places=4,blank=True,null=True)
    user = models.ForeignKey(User,verbose_name=_("out user"), on_delete=models.CASCADE,blank=True,null=True)
    status = models.CharField(_("status"),max_length=const.DB_CHAR_CODE_2,default='0',choices=STATUS)
    execute_time = models.DateTimeField(_("execute time"),blank=True,null=True)
...
class InOutDetail(models.Model):
    ...
    material = models.ForeignKey(Material,verbose_name=_("material"), on_delete=models.CASCADE,limit_choices_to={"is_virtual":"0"},blank=True,null=True)
